...
- Completed marks appear only in the 「已完成」 category (strikethrough and/or a gray filter). They must not appear in 「全部」 or in any tag filter. Completing a mark moves it there; uncompleting returns it to the normal list and tags. The 「已完成」 chip sits to the right of the last user tag (after 全部 and tags).
- Each mark can attach a location (lat/lng, and a readable place name). Default on when creating or editing. Use GPS or last-known location. Reverse-geocode with Nominatim (OpenStreetMap) using a MarkQ User-Agent, cached and rate-limited; if that fails, Android Geocoder, then raw coordinates. Request runtime location permission; if denied, still allow saving without location. Do not block save waiting for geocode. Location syncs with the entry over WebDAV. Tapping a mark’s location (address or coords) opens an in-app chooser for 百度地图 and 高德地图 only; each is launched with its official lat/lng URI (not an encoded address query). 百度 uses `map/marker` (show the pin, do not start turn-by-turn). If the chosen app is not installed, tell the user (未安装百度地图 / 未安装高德地图). Do not use a system Intent chooser, Google Maps, or generic `geo:`. If there is no location, do nothing.
- The top/app bar stays compact (short height, no extra vertical padding).
- Background, top/app bar, and + FAB colors are user-adjustable in Settings and stored on the device. Defaults: top bar green (`#0B6E4F`), background white, uncolored cards green border with shadow, + button white with shadow. If a mark has an editable color, that color is the card border (muted when completed). Per-entry card colors stay on the entry and still sync.
