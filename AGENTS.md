...
- Each mark can attach a location (lat/lng, and a readable place name). Default on when creating or editing. Use GPS or last-known location. Reverse-geocode with Nominatim (OpenStreetMap) using a MarkQ User-Agent, cached and rate-limited; if that fails, Android Geocoder, then raw coordinates. Request runtime location permission; if denied, still allow saving without location. Do not block save waiting for geocode. Location syncs with the entry over WebDAV. Tapping a mark’s location (address or coords) opens a system map-app chooser (`geo:`) for navigation; if there is no location, do nothing.
